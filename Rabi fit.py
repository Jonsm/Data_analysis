...
fname = '053018_Rabi_YOKO_1.188mA_Cav7.5613GHz_-20dBm_Qubit0.6342GHz_25dBm_Count20_TimeStep75_Avg20000.h5'
...
#
#########################################################################################################
guess = ([np.max(phase)-np.min(phase), 0.5/abs(time[np.argmax(phase)] - time[np.argmin(phase)]), 0, 0, 1e3])
opt,cov = curve_fit(func, time, phase, guess)
err = np.sqrt(abs(np.diag(cov)))
time_nice = np.linspace(-100, pts_num*time_step, pts_num*100)
